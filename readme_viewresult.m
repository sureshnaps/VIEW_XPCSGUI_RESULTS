...
settings.Batch              = [1];% Batch numbers to plot. Call be multiple batches. Default: 1
% for static 
...
settings.StaticSelectPHI    = 4;    % Q row where phi will be plotted. Valid when StaticSelect=2. Default: 1
...
settings.DynamicSelectPHI = 5;      % Q row where phi will be plotted. Valid when DynamicSelect=2. Default: 1
settings.DynamicG2XScale = 1;       % G2 plot x scale 1/2: linear/log10
settings.DynamicXScale = 1;         % 1/2: linear/log10 for x-axis (not for g2 plots). Default: 2
settings.DynamicFitting = 4;        % 1/2/3/4: display single/stretched/both/none fittings. Default: 3
settings.DynamicG2PlotErrorbars = 1;    % display G2 error bars 1/2: yes/or
...
%viewresult('B013_DGEL1_with_osc_Sq1_002_0001-0266.hdf');
viewresult('B014_DGEL1_bigger_osc_Sq1_003_0001-0266.hdf',settings);
...
%feval(get(findall(hFigViewresult,'tag','viewresult_PushbuttonExportAvgDynamic'),'callback'),'','','zzzz.mat');     % export averaged dynamic result. Last input argument is the file name (including path) to be saved; the input arguments can be left none and the default file name will be used with '_avg' added to the end of the file name.
