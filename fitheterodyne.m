function [fit2data,baseline,contrast,gamma,exponent,baseline_err,contrast_err,gamma_err,exponent_err,resnorm]= ...
                fit2heterodyne_g2s(varargin)
%                     fit2stretchedexp(delay,g2,g2Err)

delay   = varargin{1};
g2      = varargin{2};
g2Err   = zeros(size(g2));%varargin{3};
...
%     FIT2_fix are the params to be kept fixed
    ...
    [fit2,resnorm,residual,exitflag,output,lambda,jacobian]= ...
        lsqcurvefit(@fit_heterodyne,FIT2_vary,xdata,ydata,FIT2_lb1,FIT2_ub1,options, ...
        FIT2_fix,FIT2_Flag);
...
elseif (nargin == 3)
    ...
    [fit2,sig2,fit2_resnorm,fit2_flag,fit2data] =           ...
        myeasyfit(@fit_heterodyne1,xdata,ydata,edata    ...
...
% =========================================================================
function F = fit_heterodyne(x0,xdata,FIT2_fix,FIT2_Flag)
%%%used with the flexible fitting routine
% ---
% --- x(1) : Baseline
% --- x(2) : Contrast
% --- x(3) : degree of mixing
% --- x(4) : tau
% --- x(5) : exponent
% --- x(6) : omega
% ---
...
F = x(1) + x(2)*(1-x(3)).^2+x(3).^2*x(2)*exp( -1 * (xdata/x(4)).^x(5)).^2+...
    2*x(3)*(1-x(3))*x(2)*cos(x(6)*xdata)*exp( -1 * (xdata/x(4)).^x(5));
return
% =========================================================================

function F = fit_heterodyne1(x,xdata)
